...
def _copy_linked_assets(spec: LabelSpec, report: Report, destination: Path) -> dict[str, Any]:
    """Copy validated local SVG raster dependencies without following links or escaping."""
    ...
    for image in images:
        ...
        if not _is_package_path(relative):
            raise ValueError(f"Unsafe linked artwork path: {relative}")
        candidate = source_root / relative
        if candidate.is_symlink():
            raise ValueError(f"Linked artwork file is missing or unsafe: {relative}")
        ...
        try:
            ...
            raise ValueError(f"Unsafe linked artwork path: {relative}") from error
        target = destination / relative
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(source, target)
        assets[relative] = {**_manifest_entry(target), "file": relative}
    return assets
...
def _validate_entry(destination: Path, key: str, entry: Any, failures: list[str]) -> Path | None:
    ...
    is_linked_asset = key.startswith("linked asset:")
    valid_path = _is_package_path(filename) if is_linked_asset and isinstance(filename, str) else (
