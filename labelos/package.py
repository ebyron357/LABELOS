...
def create_package(
    ...
    artwork_assets = _copy_linked_svg_assets(spec, report, destination)
...
def verify_package(destination: Path) -> list[str]:
    ...
    if schema_version == _PACKAGE_SCHEMA_VERSION:
        assets = manifest.get("artwork_assets")
        if not isinstance(assets, dict):
            failures.append("artwork_assets manifest entry must be an object")
        else:
            for name, entry in assets.items():
                path = _validate_entry(destination, f"artwork asset:{name}", entry, failures, nested=True)
                if path is not None and (
                    not isinstance(name, str)
                    or not _is_package_relative_path(name)
                    or entry.get("file") != name
                ):
                    failures.append(f"artwork asset path is invalid: {name}")
...
def _relative_manifest_entry(path: Path, destination: Path) -> dict[str, str | int]:
    return {
...
def _copy_linked_svg_assets(spec: LabelSpec, report: Report, destination: Path) -> dict[str, dict[str, str | int]]:
    """Copy validated local SVG-linked rasters without allowing package path escapes."""
    ...
    for image in report.metadata.get("svg_linked_images", []):
        if not isinstance(image, dict) or not isinstance(image.get("file"), str):
            raise TypeError("Validation report contains an invalid linked SVG image entry")
        relative = Path(image["file"])
        if not _is_package_relative_path(relative.as_posix()):
            raise ValueError(f"Unsafe linked SVG image path: {image['file']}")
        source = spec.artwork.parent / relative
        if not _is_regular_file(source):
            raise ValueError(f"Linked SVG image is missing or is not a regular file: {relative}")
        target = destination / relative
        target.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(source, target)
        assets[relative.as_posix()] = _relative_manifest_entry(target, destination)
    return assets
...
def _validate_entry(
    ...
    is_valid_filename = _is_package_relative_path if nested else _is_package_filename
    if not isinstance(filename, str) or not is_valid_filename(filename):
        qualifier = "package-relative path" if nested else "package-relative filename"
        failures.append(f"{key} file must be a {qualifier}")
        return None
