...
def create_package(
    ...
    if spec.artwork.suffix.lower() == ".svg":
        for source in svg_linked_raster_paths(spec.artwork):
            relative = source.relative_to(spec.artwork.parent)
            target = destination / relative
            target.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(source, target)
            linked_assets[str(relative)] = _manifest_entry(target, str(relative))
...
def _manifest_entry(path: Path, package_path: str | None = None) -> dict[str, str | int]:
    return {
        "file": package_path or path.name,
...
def _validate_entry(destination: Path, key: str, entry: Any, failures: list[str]) -> Path | None:
    ...
    if not _is_regular_file(path) or any(parent.is_symlink() for parent in path.parents if parent != destination):
