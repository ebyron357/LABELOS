...
def verify_package(destination: Path) -> list[str]:
    ...
    if not _is_regular_file(manifest_path):
        return ["manifest.json is missing or is not a regular file"]
    try:
...
def _validate_report_and_spec(
    ...
    if not isinstance(spec, dict):
        ...
        return
    try:
        parsed_spec = LabelSpec.from_dict(spec, spec_path.parent)
    except (TypeError, ValueError) as error:
        failures.append(f"label-spec.json is invalid: {error}")
        return
    if parsed_spec.to_dict(artwork=artwork_path.name) != spec:
        failures.append("label-spec.json is not a canonical label specification")
    ...
    report_spec = report_metadata.get("spec") if isinstance(report_metadata, dict) else None
    if report_spec != spec:
