...
def _validate_safe_area(spec: LabelSpec, report: Report) -> None:
    """Ensure visible artwork stays inside the configured post-trim safe area.

    A uniform full-bleed background is intentionally excluded. Artwork with a
    non-uniform edge cannot be distinguished from content in the unsafe region,
    so it fails closed rather than claiming a safe-area check passed.
    """
    ...
    left, top, right, bottom = (
        spec.bleed_mm + spec.safe_area_mm,
        spec.bleed_mm + spec.safe_area_mm,
        spec.bleed_mm + spec.width_mm - spec.safe_area_mm,
        spec.bleed_mm + spec.height_mm - spec.safe_area_mm,
    )
    x0, y0, x1, y1 = bounds
    occupied = {
...
def _occupied_bounds_px(image) -> tuple[int, int, int, int] | None:
    """Return non-background pixel bounds, or None for an ambiguous edge."""
