...
def _svg_linked_raster_path(artwork: Path, href: str) -> Path | None:
    """Resolve a local linked SVG raster without allowing SVG to escape its bundle."""
    if href.startswith("data:"):
        return None
    parsed = urlsplit(href)
    if parsed.scheme or parsed.netloc or parsed.query or parsed.fragment or "\\" in href:
        raise ValueError("linked image must be a plain relative local path")
    reference = Path(unquote(parsed.path))
    if not parsed.path or reference.is_absolute() or ".." in reference.parts:
        raise ValueError("linked image must be a plain relative local path")
    root = artwork.parent.resolve()
    candidate = artwork.parent / reference
    try:
        resolved = candidate.resolve(strict=True)
        resolved.relative_to(root)
    except (OSError, ValueError) as error:
        raise ValueError("linked image is missing or escapes the artwork directory") from error
    current = artwork.parent
    for part in reference.parts:
        current /= part
        if current.is_symlink():
            raise ValueError("linked image path must not use symlinks")
    if not resolved.is_file():
        raise ValueError("linked image is not a regular file")
    return resolved
    ...
    for index, image in enumerate(images, start=1):
        ...
        try:
            ...
            if linked_path is None:
                ...
                relative_path = linked_path.relative_to(spec.artwork.parent.resolve()).as_posix()
                linked_images.append({**metadata, "file": relative_path})
