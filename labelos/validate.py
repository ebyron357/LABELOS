...
def _validate_svg_rasters(
    ...
    for index, image in enumerate(images, start=1):
        ...
        if href is None:
            ...
            continue
        is_linked = not href.startswith("data:image/")
        try:
            data = _svg_embedded_raster_data(href)
            if data is None:
                raster, relative_path = _open_linked_svg_raster(spec.artwork, href)
            else:
                from PIL import Image

                raster = Image.open(BytesIO(data))
                relative_path = None
            with raster:
                raster.load()
                pixels = raster.size
            ...
            if relative_path is not None:
                linked_images.append({"index": index, "file": relative_path.as_posix()})
...
def _open_linked_svg_raster(artwork: Path, href: str):
    """Open a locally linked raster without following links outside the artwork tree."""

    from PIL import Image

    parsed = urlparse(href)
    if parsed.scheme or parsed.netloc:
        raise ValueError("linked image must use a local relative path")
    encoded_path = unquote(parsed.path)
    if not encoded_path:
        raise ValueError("linked image path is empty")
    path = Path(encoded_path)
    if path.is_absolute() or any(part == ".." for part in path.parts):
        raise ValueError("linked image path must stay below the SVG directory")
    # Backslashes are separators on Windows and would make a package unsafe there.
    if "\\" in encoded_path:
        raise ValueError("linked image path must not contain backslashes")
    root = artwork.parent.resolve()
    candidate = root / path
    try:
        relative_path = candidate.resolve().relative_to(root)
    except ValueError as error:
        raise ValueError("linked image path must stay below the SVG directory") from error
    if candidate.is_symlink() or not candidate.is_file():
        raise ValueError("linked image must be a readable regular file")
    return Image.open(candidate), relative_path
