...
def _linked_svg_raster_path(artwork: Path, href: str) -> tuple[Path, str]:
    """Return a safe local raster reference and its stable relative path.

    Release packages preserve SVG-relative asset paths, so links must be local,
    below the artwork directory, and free of symlinks. Remote URLs and fragments
    are deliberately unsupported: their bytes cannot be validated or packaged.
    """

    parsed = urlsplit(href)
    if parsed.scheme or parsed.netloc or parsed.query or parsed.fragment:
        raise ValueError("linked image must be a plain local path")
    relative = Path(href)
    if relative.is_absolute() or not href or any(part in {"", ".", ".."} for part in relative.parts):
        raise ValueError("linked image path must be a safe relative path")
    ...
    for parent in (candidate, *candidate.parents):
        ...
        if parent.is_symlink():
            raise ValueError("linked image path must not contain a symlink")
    resolved = candidate.resolve()
    try:
        resolved.relative_to(root)
    except ValueError as error:
        raise ValueError("linked image must remain below the artwork directory") from error
...
def _validate_svg_embedded_rasters(
    ...
    for index, image in enumerate(images, start=1):
        ...
        try:
            ...
            if data is None:
                raster_path, source_relative = _linked_svg_raster_path(spec.artwork, href)
