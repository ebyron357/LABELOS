...
class SvgLinkedRaster:
    """A local, regular raster file referenced by an SVG image element."""
...
def svg_linked_rasters(artwork: Path) -> list[SvgLinkedRaster]:
    """Resolve SVG-linked rasters without permitting external or unsafe paths."""
...
def _has_symlink_component(path: Path, root: Path) -> bool:
    """Return whether a path traverses a symlink at or below its trusted root."""
...
def _validate_svg_embedded_rasters(
    ...
    try:
        linked_by_href = {asset.href: asset for asset in svg_linked_rasters(spec.artwork)}
    except ValueError as error:
        report.add("SVG_LINKED_IMAGE_INSPECTION_FAILED", "error", str(error))
        return
    ...
    for index, image in enumerate(images, start=1):
        ...
        try:
            ...
            if data is None and href.startswith("data:image/"):
                # Nested SVG is vector artwork, not a placed raster to which DPI applies.
                continue
            linked = linked_by_href.get(href)
            if data is None and linked is None:
                raise ValueError("linked image was not resolved")
            from PIL import Image

            with Image.open(BytesIO(data) if data is not None else linked.path) as raster:
                raster.load()
                pixels = raster.size
                ...
                linked_images.append(
                    {
                        **metadata,
                        "href": href,
                        "sha256": hashlib.sha256(linked.path.read_bytes()).hexdigest(),
                    }
                )
            else:
                embedded_images.append(metadata)
            if effective_dpi < spec.min_dpi:
                report.add(
                    "SVG_EMBEDDED_IMAGE_DPI_TOO_LOW" if data is not None else "SVG_LINKED_IMAGE_DPI_TOO_LOW",
                    "error",
