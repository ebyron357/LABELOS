...
def _validate_safe_area(spec: LabelSpec, report: Report) -> None:
    """Reject visible artwork content that falls outside the configured safe area."""
    ...
    try:
        ...
        report.add(
            "SAFE_AREA_UNCHECKED",
            "error",
            f"Could not inspect artwork safe area: {error}",
        )
        return
    if bounds is None:
        return
...
def _visible_bounds(image) -> tuple[int, int, int, int] | None:
    """Return non-canvas pixel bounds; a blank uniform canvas has no visible bounds."""
