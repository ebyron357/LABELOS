...
def _svg_linked_raster_path(artwork: Path, href: str) -> tuple[Path, str]:
    """Resolve a local SVG raster reference without allowing filesystem escape."""

    parsed = urlsplit(href)
    if parsed.scheme or parsed.netloc or parsed.query or parsed.fragment:
        raise ValueError("image href must be a plain relative local path")
    if not parsed.path:
        raise ValueError("image href path is empty")

    root = artwork.parent.resolve()
    candidate = artwork.parent / parsed.path
    if candidate.is_symlink():
        raise ValueError("image href must not be a symbolic link")
    ...
    if not resolved.is_file():
        raise ValueError("image href is missing or is not a regular file")
    ...
    for index, image in enumerate(images, start=1):
        ...
        try:
            ...
            if linked_path is None:
                ...
                linked_images.append({**image_metadata, "file": relative_path})
            if effective_dpi < spec.min_dpi:
                report.add(
                    "SVG_EMBEDDED_IMAGE_DPI_TOO_LOW" if linked_path is None else "SVG_LINKED_IMAGE_DPI_TOO_LOW",
                    "error",
                    f"{'Embedded' if linked_path is None else 'Linked'} image {index} has effective resolution "
            ...
            report.add(
                ...
                "error",
                f"Could not inspect {'embedded' if href.startswith('data:') else 'linked'} image {index}: {error}",
            )
    if inspected_images:
