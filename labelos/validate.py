...
def svg_linked_raster_paths(artwork: Path) -> list[Path]:
    """Return safe local raster references from an SVG, rejecting unsafe references.

    A production package must be self-contained. Only relative files below the SVG
    directory are supported; URLs, fragments, query strings, traversal, and symlinks
    would make validation and release contents non-reproducible.
    """
    ...
    for index, image in enumerate(images, start=1):
        ...
        if href is None or href.startswith("data:image/"):
            continue
        paths.append(_svg_linked_raster_path(artwork, href, index))
    return paths


def _svg_linked_raster_path(artwork: Path, href: str, index: int) -> Path:
    if not href or "\\" in href or "?" in href or "#" in href or "://" in href:
        raise ValueError(f"Linked image {index} must use a local relative path")
    relative = Path(href)
    if relative.is_absolute() or any(part in {"", ".", ".."} for part in relative.parts):
        raise ValueError(f"Linked image {index} must use a local relative path")
    candidate = artwork.parent
    for part in relative.parts:
        candidate /= part
        if candidate.is_symlink():
            raise ValueError(f"Linked image {index} must not use a symlink")
    candidate = candidate.resolve()
    try:
        candidate.relative_to(artwork.parent.resolve())
    except ValueError as error:
        raise ValueError(f"Linked image {index} escapes the SVG directory") from error
...
def _validate_svg_embedded_rasters(
    ...
    for index, image in enumerate(images, start=1):
        href = image.get("href") or image.get("{http://www.w3.org/1999/xlink}href")
        if href is None:
            report.add("SVG_RASTER_INSPECTION_FAILED", "error", f"Image {index} has no href")
            continue
        try:
            ...
            if data is None:
                linked_path = _svg_linked_raster_path(spec.artwork, href, index)
            ...
            if data is None:
                details.update(
                    {
                        "file": str(linked_path.relative_to(spec.artwork.parent.resolve())),
                        "sha256": hashlib.sha256(linked_path.read_bytes()).hexdigest(),
                    }
                )
                linked_images.append(details)
            else:
                embedded_images.append(details)
            if effective_dpi < spec.min_dpi:
                report.add(
                    "SVG_LINKED_IMAGE_DPI_TOO_LOW" if data is None else "SVG_EMBEDDED_IMAGE_DPI_TOO_LOW",
                    "error",
                    f"{'Linked' if data is None else 'Embedded'} image {index} has effective resolution {effective_dpi:.1f} DPI; "
                    f"minimum is {spec.min_dpi} DPI",
                )
        except (ImportError, OSError, ValueError) as error:
            report.add(
                "SVG_LINKED_IMAGE_INSPECTION_FAILED" if not href.startswith("data:image/") else "SVG_EMBEDDED_IMAGE_INSPECTION_FAILED",
                "error",
                f"Could not inspect image {index}: {error}",
            )
    if embedded_images:
