...
def test_safe_area_accepts_full_bleed_background_with_inset_png_content(tmp_path):
    artwork = tmp_path / "safe.png"
    image = Image.new("RGB", (1060, 560), "white")
    ImageDraw.Draw(image).rectangle((80, 80, 300, 180), fill="black")
    image.save(artwork)
    spec = LabelSpec.from_dict(
        {
            "artwork": artwork.name,
