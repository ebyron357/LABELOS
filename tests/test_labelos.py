...
def test_linked_svg_raster_low_dpi_fails(tmp_path):
    ...
    artwork = tmp_path / "label.svg"
    artwork.write_text(_linked_raster_svg("source.png"), encoding="utf-8")
    spec = LabelSpec.from_dict(
        {"artwork": artwork.name, "width_mm": 100, "height_mm": 50, "min_dpi": 300}, tmp_path
    )
...
@pytest.mark.parametrize(
    "href",
    ["../outside.png", "/tmp/outside.png", "https://example.test/raster.png", "source.png?x=1"],
)
...
def test_package_refuses_artwork_or_linked_assets_changed_after_validation(tmp_path):
    raster = tmp_path / "source.png"
    Image.new("RGB", (600, 600), "black").save(raster)
    artwork = tmp_path / "label.svg"
    artwork.write_text(_linked_raster_svg("source.png"), encoding="utf-8")
    ...
    with pytest.raises(ValueError, match="Linked SVG assets changed after validation"):
        create_package(spec, report, tmp_path / "release")
    artwork.write_text(_linked_raster_svg("source.png") + " ", encoding="utf-8")

    with pytest.raises(ValueError, match="Artwork changed after validation"):
        create_package(spec, report, tmp_path / "release-artwork")
