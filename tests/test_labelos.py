...
def test_svg_unsupported_safe_area_primitive_fails_closed(tmp_path):
    artwork = tmp_path / "path.svg"
    artwork.write_text(
        '<svg xmlns="http://www.w3.org/2000/svg" width="106mm" height="56mm" viewBox="0 0 106 56">'
        '<path d="M0 0 H106 V56 Z"/></svg>',
        encoding="utf-8",
    )
    spec = LabelSpec.from_dict(
        {"artwork": artwork.name, "width_mm": 100, "height_mm": 50, "bleed_mm": 3, "safe_area_mm": 2},
    ...
    assert not report.passed
    assert report.issues[-1].code == "SAFE_AREA_UNCHECKABLE"
