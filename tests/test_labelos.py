...
def _linked_raster_svg(tmp_path, href: str, name: str = "linked.svg") -> Path:
    artwork = tmp_path / name
    artwork.write_text(
        (
...
def test_linked_svg_raster_is_checked_and_packaged_with_its_relative_path(tmp_path):
    images = tmp_path / "images"
    images.mkdir()
    source = images / "photo.png"
    Image.new("RGB", (600, 600), "black").save(source)
    artwork = _linked_raster_svg(tmp_path, "images/photo.png")
    spec = LabelSpec.from_dict(
    ...
    assert report.passed
    assert report.metadata["svg_linked_images"][0]["file"] == "images/photo.png"
    assert report.metadata["svg_linked_images"][0]["dpi"] == 600
    assert manifest["schema_version"] == 2
    assert manifest["linked_assets"][0]["file"] == "images/photo.png"
    assert (manifest_path.parent / "images" / "photo.png").read_bytes() == source.read_bytes()
    assert not verify_package(manifest_path.parent)


def test_linked_svg_raster_dpi_is_enforced(tmp_path):
    source = tmp_path / "photo.png"
    Image.new("RGB", (72, 72), "black").save(source)
    artwork = _linked_raster_svg(tmp_path, source.name)
    spec = LabelSpec.from_dict(
        {"artwork": artwork.name, "width_mm": 25.4, "height_mm": 25.4, "min_dpi": 300}, tmp_path
    )
...
@pytest.mark.parametrize(
    ("href", "code"),
    [
        ("https://example.test/photo.png", "SVG_LINKED_IMAGE_UNSAFE_PATH"),
        ("../photo.png", "SVG_LINKED_IMAGE_UNSAFE_PATH"),
        ("missing.png", "SVG_LINKED_IMAGE_MISSING"),
    ],
)
def test_linked_svg_raster_requires_safe_local_source(tmp_path, href, code):
...
def test_linked_svg_raster_rejects_symlinked_source(tmp_path):
    ...
    try:
        ...
        pytest.skip("symlinks are not permitted in this environment")
    artwork = _linked_raster_svg(tmp_path, linked.name)
    spec = LabelSpec.from_dict({"artwork": artwork.name, "width_mm": 25.4, "height_mm": 25.4}, tmp_path)

    report = validate(spec)

    assert not report.passed
    assert any(issue.code == "SVG_LINKED_IMAGE_UNSAFE_PATH" for issue in report.issues)


def test_verify_package_remains_compatible_with_schema_one_manifest(tmp_path):
    manifest_path = create_package(passing_spec(), validate(passing_spec()), tmp_path / "release")
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest["schema_version"] = 1
    manifest.pop("linked_assets")
