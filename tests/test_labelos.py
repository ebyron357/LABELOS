...
def test_safe_area_allows_uniform_background_and_content_inside_margin(tmp_path):
...
def test_safe_area_violation_is_detected_in_svg(tmp_path):
    artwork = tmp_path / "unsafe.svg"
    artwork.write_text(
        (
            '<svg xmlns="http://www.w3.org/2000/svg" width="106mm" height="56mm" '
            'viewBox="0 0 106 56"><rect width="106" height="56" fill="white"/>'
            '<rect x="1" y="20" width="2" height="10" fill="black"/></svg>'
        ),
        encoding="utf-8",
    )
    spec = LabelSpec.from_dict(
...
def test_invalid_pdf_reports_error_without_crashing(tmp_path):
