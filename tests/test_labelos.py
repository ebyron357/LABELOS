...
def test_malformed_pdf_fails_closed_without_crashing(tmp_path, capsys):
        ...
        json.dumps(
            {
                ...
                "width_mm": 100,
                "height_mm": 50,
                "qr_value": "https://example.test/unreachable",
            }
        ),
        encoding="utf-8",
    )
    ...
    assert [issue["code"] for issue in output["issues"]] == ["PDF_INVALID"]
    assert output["metadata"]["spec"]["width_mm"] == 100
