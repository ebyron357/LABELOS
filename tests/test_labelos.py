...
def test_module_cli_entry_point_runs_doctor():
...
def test_linked_svg_raster_is_validated_and_packaged(tmp_path):
    ...
    assets.mkdir()
    Image.new("RGB", (1200, 1200), "black").save(assets / "logo.png")
    artwork = tmp_path / "label.svg"
    artwork.write_text(
        (
            '<svg xmlns="http://www.w3.org/2000/svg" width="100mm" height="100mm" '
            'viewBox="0 0 100 100"><image href="assets/logo.png" width="100" height="100"/></svg>'
        ),
        encoding="utf-8",
    )
    spec = LabelSpec.from_dict(
    ...
    assert json.loads(manifest.read_text(encoding="utf-8"))["linked_assets"]["assets/logo.png"]["file"] == (
        "assets/logo.png"
    )
    assert verify_package(manifest.parent) == []
    packaged_logo = manifest.parent / "assets" / "logo.png"
    packaged_logo.write_bytes(b"x" * packaged_logo.stat().st_size)
    assert verify_package(manifest.parent) == ["linked asset:assets/logo.png checksum mismatch: assets/logo.png"]


@pytest.mark.parametrize(
    ("href", "expected_code"),
    [
        ("assets/missing.png", "SVG_LINKED_IMAGE_INSPECTION_FAILED"),
        ("https://example.test/logo.png", "SVG_LINKED_IMAGE_INSPECTION_FAILED"),
        ("../outside.png", "SVG_LINKED_IMAGE_INSPECTION_FAILED"),
    ],
)
def test_unsafe_or_missing_linked_svg_raster_fails_closed(tmp_path, href, expected_code):
    artwork = tmp_path / "label.svg"
    artwork.write_text(
        (
            '<svg xmlns="http://www.w3.org/2000/svg" width="100mm" height="100mm" '
            f'viewBox="0 0 100 100"><image href="{href}" width="100" height="100"/></svg>'
        ),
        encoding="utf-8",
    )
    spec = LabelSpec.from_dict(
...
def test_low_resolution_linked_svg_raster_fails(tmp_path):
    assets = tmp_path / "assets"
    assets.mkdir()
    Image.new("RGB", (72, 72), "black").save(assets / "logo.png")
    artwork = tmp_path / "label.svg"
    artwork.write_text(
        (
            '<svg xmlns="http://www.w3.org/2000/svg" width="100mm" height="100mm" '
