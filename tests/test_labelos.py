...
def test_uniform_full_bleed_background_is_ignored_for_safe_area(tmp_path):
    artwork = tmp_path / "safe.png"
    image = Image.new("RGB", (106, 56), "white")
    ImageDraw.Draw(image).rectangle((5, 5, 100, 50), fill="black")
    image.save(artwork)
    spec = LabelSpec.from_dict(
        {
            "artwork": artwork.name,
    ...
    assert report.passed
    assert report.metadata["safe_area"]["content_bounds_px"] == {
        "left": 5,
        "top": 5,
        "right": 101,
        "bottom": 51,
...
def test_content_outside_safe_area_fails(tmp_path):
    artwork = tmp_path / "unsafe.png"
    image = Image.new("RGB", (106, 56), "white")
    ImageDraw.Draw(image).rectangle((3, 3, 100, 50), fill="black")
    image.save(artwork)
    spec = LabelSpec.from_dict(
        {
            "artwork": artwork.name,
...
def test_safe_area_applies_to_svg_artwork(tmp_path):
    ...
    artwork.write_text(
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="106mm" height="56mm" viewBox="0 0 106 56">
          ...
          <rect x="3" y="3" width="98" height="48" fill="black"/>
        </svg>
        """,
        encoding="utf-8",
    )
