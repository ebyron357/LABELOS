...
def test_linked_svg_raster_is_validated_and_packaged(tmp_path):
    ...
    artwork.write_text(
        (
            '<svg xmlns="http://www.w3.org/2000/svg" width="100mm" height="100mm" '
            'viewBox="0 0 100 100"><image href="assets/high-resolution.png" '
            'width="100" height="100"/></svg>'
        ),
        encoding="utf-8",
    )
    ...
    assert (manifest.parent / "assets" / "high-resolution.png").is_file()
    assert not verify_package(manifest.parent)


def test_linked_svg_raster_rejects_unsafe_or_low_resolution_reference(tmp_path):
    ...
    artwork.write_text(
        (
            '<svg xmlns="http://www.w3.org/2000/svg" width="100mm" height="100mm" '
            'viewBox="0 0 100 100"><image href="../low-resolution.png" '
            'width="100" height="100"/></svg>'
        ),
        encoding="utf-8",
    )
