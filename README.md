...
check. A uniform canvas color sampled from all four corners is treated as a full-bleed background;
otherwise all visible artwork is assessed conservatively.
...
- `labelos verify-package DESTINATION`: verifies manifest schema, safe regular-file paths,
  checksums, byte counts, passing validation status, and report/spec consistency.
