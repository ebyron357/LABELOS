...
code was checked. When `safe_area_mm` is configured, LABELOS renders SVG/PDF artwork at 300
DPI and checks visible pixels; PNG artwork is inspected directly. Any artwork extending into
the exclusion zone fails validation.
...
- `labelos package CONFIG DESTINATION`: validates, then writes artwork, a JSON validation
  report, an immutable copy of the label spec, and a SHA-256 manifest. Existing package
  destinations are never overwritten.
- `labelos verify-package DESTINATION`: verifies checksums, byte counts, package paths,
  regular-file status, a passing validation report, and agreement among the report, manifest,
  and packaged spec.
