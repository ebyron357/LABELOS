...
byte-count mismatches, and reports that do not record a pass. Passing SVG packages
also include safe local raster files referenced by `<image href="...">`; remote,
absolute, traversal, missing, and symlink targets are rejected during validation.
...
| `SVG_LINKED_IMAGE_DPI_TOO_LOW` | A local SVG `href` raster is below `min_dpi` |
| `SVG_LINKED_IMAGE_INSPECTION_FAILED` | A local SVG `href` is unsafe, missing, or unreadable |
