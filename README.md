...
- `labelos package CONFIG DESTINATION`: validates, then writes artwork, a JSON validation
  report, a package-local canonical `label-spec.json`, and a SHA-256 manifest. Existing package
  destinations are never overwritten.
- `labelos verify-package DESTINATION`: verifies checksums, byte counts, manifest schema,
  package-local regular-file paths, and that the report records a passing validation for the
...
Malformed PDFs are reported as `PDF_INVALID` validation errors; they do not crash the operator
CLI.
