...
`safe_area_mm` is set, non-background artwork must remain inside the inset formed by
`bleed_mm + safe_area_mm`; a uniform full-bleed background is allowed. SVG, PNG, and PDF
artwork are accepted. PDF inspection and QR/barcode decoding are installed with LABELOS; SVG
and PDF are rendered at 300 DPI before code decoding. If `barcode_value` or `qr_value` is
configured but the decoder cannot load, validation fails rather than asserting a code was checked.

## Commands

- `labelos validate CONFIG [--json]`: validate format, dimensions, raster resolution,
  configured safe areas, required copy, and configured barcode/QR values.
...
- `labelos verify-package DESTINATION`: verifies checksums, byte counts, report/spec
  consistency, safe regular-file paths, and unexpected package files.
