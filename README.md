...
byte-count mismatches, unmanifested SVG-linked rasters, and reports that do not record
a pass. SVG `<image href>` raster files must be local regular files below the SVG
directory; the package retains their relative paths and checksums.
...
| `SVG_RASTER_IMAGE_INSPECTION_FAILED` | Placed SVG raster is missing, unsafe, or unreadable |
