...
- `labelos package CONFIG DESTINATION`: validates, then writes artwork, a JSON validation
  report, the normalized label specification, and a SHA-256 manifest. Existing package
  destinations are never overwritten.
- `labelos verify-package DESTINATION`: verifies checksums and byte counts, exact package
  contents, regular-file safety, the spec-to-artwork binding, and a passing report.
