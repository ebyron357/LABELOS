...
Artwork dimensions include bleed: the example above expects a 106 × 56 mm asset. When a
safe area is configured, LABELOS rasterizes the artwork at 300 DPI and rejects non-background
content between the trim edge and that inset. PNG transparency or ambiguous corner backgrounds
fail this check closed. SVG, PNG, and PDF artwork are accepted. PDF inspection and QR/barcode
decoding are installed with LABELOS; SVG and PDF are rendered at 300 DPI before code decoding. If `barcode_value` or
...
- `labelos validate CONFIG [--json]`: validate format, dimensions, raster resolution,
  safe-area clearance, required copy, and configured barcode/QR values.
