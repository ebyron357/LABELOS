...
- `labelos package CONFIG DESTINATION`: validates, then writes artwork, a canonical
  package-local label specification, a JSON validation report, and a SHA-256 manifest.
  Existing package destinations are never overwritten.
- `labelos verify-package DESTINATION`: fails closed unless every package artifact is a
  regular package-local file with a lowercase SHA-256 digest and matching byte count, the
  report records a pass, and the report/spec/manifest agree on the physical specification.
