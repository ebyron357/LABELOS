...
code was checked. When `safe_area_mm` is configured, visible artwork must remain inside the
trim plus safe-area inset; an unavailable renderer is a validation error.
...
- `labelos package CONFIG DESTINATION`: validates, then writes artwork, a JSON validation
  report, serialized label specification, and a SHA-256/byte-count manifest. Existing package
  destinations are never overwritten.
- `labelos verify-package DESTINATION`: verifies checksums, sizes, regular-file entries, and
  consistency between the manifest, specification, and validation report.
