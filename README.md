...
- `labelos package CONFIG DESTINATION`: validates, then writes artwork, a JSON validation
  report, and a schema-v2 SHA-256 manifest. Existing package destinations are never
  overwritten.
- `labelos verify-package DESTINATION`: verifies manifest structure, approved checksum
  entries, regular package files, byte counts, a passing report consistent with the packaged
  specification, and rejects untracked release files.
