...
- `labelos package CONFIG DESTINATION`: validates, then writes artwork, a JSON validation
  report, a package-local canonical `label-spec.json`, and a SHA-256 manifest. Existing
  package destinations are never overwritten.
- `labelos verify-package DESTINATION`: verifies manifest schema, safe package-local file
  names, byte counts, and SHA-256 checksums for the artwork, report, and label spec.
