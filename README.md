...
byte-count mismatches, incomplete linked SVG assets, and reports that do not record a pass.
Linked SVG raster images must use regular, local paths below the SVG's directory. They are
resolution-checked, copied with their relative paths into the release package, and recorded
in the manifest; network URLs, symlinks, traversal paths, and missing files fail validation.
...
| `SVG_LINKED_IMAGE_INSPECTION_FAILED` | A linked SVG raster is missing, unsafe, or unreadable |
