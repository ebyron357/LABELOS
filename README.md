...
SVG `<image>` elements may use embedded data URIs or relative local raster files.
Linked files must be regular files below the SVG directory; remote URLs, traversal,
and symlinks are rejected. Release packages preserve approved linked assets and
...
| `SVG_LINKED_IMAGE_UNSAFE` | SVG linked image is not a safe local regular file |
