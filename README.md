...
The installed `labelos` command and `python -m labelos` run the same operator CLI.
...
| `SVG_LINKED_IMAGE_INSPECTION_FAILED` | A linked SVG raster is missing, unsafe, unreadable, or uses a symlink |
