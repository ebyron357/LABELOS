...
code was checked. When `safe_area_mm` is nonzero, LABELOS also renders the artwork and rejects
visible non-background content outside the rectangle inset by `bleed_mm + safe_area_mm`.
Uniform full-bleed canvas backgrounds are ignored for this check.

## Commands

- `labelos validate CONFIG [--json]`: validate format, dimensions, raster resolution,
  required copy, safe-area content bounds, and configured barcode/QR values.
