...
Use `python3 -m labelos` below so the workflow does not depend on the console-script
directory being present in `PATH`. The equivalent `labelos` commands work after a
standard installation when that directory is configured.
...
| `SVG_EMBEDDED_IMAGE_DPI_TOO_LOW` / `SVG_LINKED_IMAGE_DPI_TOO_LOW` | Embedded or safely linked SVG raster is below `min_dpi` |
| `SVG_LINKED_IMAGE_INSPECTION_FAILED` | SVG linked raster is unsafe, missing, unreadable, or cannot be inspected |
