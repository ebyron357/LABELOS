...
- `labelos package CONFIG DESTINATION`: validates, then writes the artwork, normalized label
  specification, JSON validation report, and a SHA-256/byte-count manifest. Existing package
  destinations are never overwritten.
- `labelos verify-package DESTINATION`: verifies every package artifact, rejects untracked or
  symlinked files, checks the packaged specification binds to its artwork, and confirms that the
  packaged validation report passed.
