...
and PDF artwork are accepted. PDF inspection checks page count, physical dimensions, font
resources, and the effective DPI of every placed raster image. SVG and PDF are rendered at 300
DPI before code decoding. If `barcode_value` or `qr_value` is configured but the decoder cannot
load, validation fails rather than asserting a code was checked. Invalid PDFs also fail closed.
