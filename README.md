...
- `labelos verify-package DESTINATION`: verifies artifact checksums and sizes, rejects
  unexpected, non-regular, duplicate, or unsafe artifact names, and confirms that the packaged
  spec binds to the artwork and that the report passed.
...
Release packages use manifest schema version 2. They contain exactly `manifest.json`, the
artwork, `label-spec.json`, and `validation-report.json`; every packaged artifact is recorded
with its filename, byte count, and SHA-256 checksum.
