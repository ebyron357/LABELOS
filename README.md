...
- `labelos package CONFIG DESTINATION`: validates, then writes artwork, a JSON validation
  report, and a versioned SHA-256 manifest. Existing package destinations are never overwritten.
- `labelos verify-package DESTINATION`: verifies the manifest schema, checksums, byte counts,
  safe paths, the passing report/spec relationship, and rejects unexpected package files.
