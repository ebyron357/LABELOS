...
- `labelos verify-package DESTINATION`: verifies the schema-v2 manifest, expected files,
  regular-file/symlink safety, byte counts, SHA-256 checksums, and the passing validation report.
...
Release packages use manifest schema version 2. `verify-package` rejects legacy manifests,
unsafe paths, symlinks, unexpected files, altered byte counts or checksums, and reports that do
not pass or no longer match the packaged specification.
