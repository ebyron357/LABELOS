...
byte-count mismatches, reports that do not record a pass, and release assets that do
not match the validation report. SVG raster files referenced with a plain relative
`href` are checked at their placed resolution, hashed during validation, included in
the release package, and verified with the package. Remote URLs, absolute paths,
path traversal, symlinks, and missing linked rasters fail validation.
...
| `SVG_LINKED_IMAGE_DPI_TOO_LOW` | A linked SVG raster is below `min_dpi` |
