...
| `SVG_LINKED_IMAGE_INSPECTION_FAILED` | SVG raster link is unsafe, missing, or unreadable | Use a regular local raster below the SVG directory; do not use remote, absolute, traversal, query, fragment, or symlink paths |
...
| `RELEASE_VERIFICATION_REQUIRED` | Package was not verified or changed after verification | Re-run `verify-package`, obtain approval again if needed, and release only an intact package |
