...
| Linked (non-embedded) SVG raster files | **AVAILABLE NOW** (only safe local relative raster links; DPI-checked and included in release packages) |
...
Verified on 2026-09-06 from branch `cursor/label-production-system-readiness-ee09`:
implementation revision `dce87e3`:
...
python3 -m labelos validate /tmp/labelos-linked-cli/spec.json --json  # PASS
python3 -m labelos package /tmp/labelos-linked-cli/spec.json /tmp/labelos-linked-cli/release --json
python3 -m labelos verify-package /tmp/labelos-linked-cli/release --json  # PASS
# linked SVG raster is copied to release/assets/raster.png and checksummed in schema 2 manifest
