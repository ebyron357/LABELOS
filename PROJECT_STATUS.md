...
- Schema-v2 release directories containing copied artwork, normalized `label-spec.json`,
  validation report, manifest, SHA-256 checksums, and byte counts.
- Fail-closed package verification that rejects unsafe or duplicate artifact names, unexpected
  files, symbolic links and other nonregular artifacts, invalid manifest metadata, spec/artwork
  binding failures, and reports that did not pass.
- Unreadable PDF artwork is reported as `PDF_INVALID` rather than crashing the CLI.
...
Verified on 2026-08-15 from commit `0ffa37f7c2186bbb5bd11eddfb73b69625af5fdc`:
...
Results: 11 tests passed; Ruff and bytecode compilation passed; the sdist and wheel were generated in `dist/`; and the
