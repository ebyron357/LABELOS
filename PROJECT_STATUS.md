...
- Immutable-style release directories containing copied artwork, validation report, manifest,
  SHA-256 checksums, byte counts, and strict verification against malformed manifests,
  symlinks, path traversal, and untracked files.
...
Verified on 2026-08-17 from implementation commit `6c328a6c32479bb21ed4edf384fa819a0a586931`:

```text
python3 -m pytest -q                      # 14 passed
python3 -m ruff check .                   # passed
python3 -m compileall -q labelos tests    # passed
python3 -m build --outdir /tmp/labelos-build-799f
...
in `/tmp/labelos-build-799f`; and the end-to-end package was created and checksum-verified.
`doctor` confirmed PyMuPDF and ZXing-C++ are available; Callas pdfToolbox remains unavailable.
QR and Code 128 regression tests generate raster, SVG, and PDF fixtures and verify their
decoded expected values. Safe-area regression tests accept in-bounds content and reject
out-of-bounds PNG, SVG, and PDF content. GitHub Actions runs tests, lint, and builds on Python
3.10 and 3.12.
