...
| Linked (non-embedded) SVG raster files | **AVAILABLE NOW** (local relative regular files are DPI-checked, packaged, and integrity-verified; remote, absolute, traversal, missing, and symlinked paths fail closed) |
...
Verified on 2026-08-19 from the current production-readiness worktree:

```text
python -m pytest -q                      # 58 passed (one third-party deprecation warning)
...
The operator CLI is also available path-independently as `python -m labelos`.
