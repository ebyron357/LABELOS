...
| Linked local SVG raster files | **AVAILABLE NOW** (plain non-symlink relative paths beneath the SVG directory are decoded, DPI-checked, and packaged) |
...
The CLI flow and full suite were verified after the linked-SVG-raster support added
on 2026-09-14. A schema-2 release package now records those copied assets with their
original relative paths and checksums; schema-1 packages remain verifiable. Evidence:
`/opt/cursor/artifacts/labelos_linked_raster_verification_final2_2026_09_14.txt`.
External/human blockers remain Callas licensing and profile configuration, printer
approval, Illustrator workstation/template, and the product decision for OCR of
outlined/raster-only required copy.
