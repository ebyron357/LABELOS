...
- Safe-area enforcement for SVG/PDF renderings; flattened PNG safe-area checks fail closed
  because critical content cannot be separated reliably from an opaque background.
...
- Immutable-style release directories containing copied artwork, validation report, manifest,
  portable label specification, SHA-256 checksums, and byte counts.
- Package verification rejects malformed manifests, symlinks, path traversal, missing artifacts,
  checksum/byte-count mismatches, and disagreement between the manifest/report/specification.
...
Verified on 2026-08-18 from code commit `c809f74`:
...
python3 -m build --outdir /tmp/.../build  # sdist and wheel created
...
Results: 14 tests passed; Ruff, compileall, and pip check passed; the sdist and wheel were
generated in `/tmp/labelos-final-oRzyZT/build`; and the end-to-end package was created and
checksum-verified at `/tmp/labelos-final-oRzyZT/package`.
`doctor` confirmed PyMuPDF and ZXing-C++ are available; Callas pdfToolbox remains unavailable.
QR and Code 128 regression tests generate raster, SVG, and PDF fixtures and verify their
decoded expected values. Safe-area, PDF-image-DPI, and release-manifest tampering regressions
are also covered. GitHub Actions runs tests, lint, and builds on Python 3.10 and 3.12.
