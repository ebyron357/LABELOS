...
- Malformed PDF artwork fails closed with a structured `PDF_INVALID` validation error rather
  than a traceback.
...
python3 -m labelos.cli package examples/label.json /tmp/labelos-e2e-current --json
python3 -m labelos.cli verify-package /tmp/labelos-e2e-current --json
python3 -m labelos.cli doctor --json
```

Results: 10 tests passed; Ruff, bytecode compilation, and dependency checks passed; the sdist
and wheel were generated in `dist/`; and the end-to-end package was created and
checksum-verified at `/tmp/labelos-e2e-current`. The malformed-PDF unit and CLI regression
confirmed that invalid input produces `PDF_INVALID` and exits nonzero without a traceback.
