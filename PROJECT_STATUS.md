...
- Immutable-style release directories containing copied artwork, canonical label specification,
  validation report, manifest, SHA-256 checksums, and byte counts.
- Package verification rejects unsupported manifest schemas and filename traversal before reading
  package entries.
...
Verified on 2026-08-10 from implementation commit `248f1ad8161f2941010fcb6c2da610cdd3614833`:
...
python3 -m labelos.cli package examples/label.json /tmp/labelos-release-e2e --json
python3 -m labelos.cli verify-package /tmp/labelos-release-e2e --json
python3 -m labelos.cli doctor --json
```

Results: 10 tests passed; Ruff passed; the sdist and wheel were generated in `dist/`; and the
end-to-end package was created and checksum-verified at `/tmp/labelos-release-e2e`. The package
contains a checksummed `label-spec.json`; its manifest contains exactly artwork, validation-report,
and label-spec entries with verified byte counts. Regression coverage rejects manifest path escape.
