...
- Release-package verification rejects malformed schema versions, unsafe package filenames,
  malformed or mismatched checksums/byte counts, non-passing validation reports, and
  disagreement between the packaged artwork, specification, and report.
- Invalid PDF input is reported as `PDF_INVALID` instead of terminating validation.
...
Verified on 2026-08-10 from commit `86b59ae`:
...
python3 -m labelos.cli package examples/label.json /tmp/labelos-e2e-4436 --json
python3 -m labelos.cli verify-package /tmp/labelos-e2e-4436 --json
python3 -m labelos.cli doctor --json
```

Results: 12 tests passed; Ruff passed; the sdist and wheel were generated in `dist/`; and the
end-to-end package was created and schema/checksum-verified at `/tmp/labelos-e2e-4436`.
`doctor` confirmed PyMuPDF and ZXing-C++ are available; Callas pdfToolbox remains unavailable.
QR and Code 128 regression tests generate raster, SVG, and PDF fixtures and verify their
decoded expected values. Package regressions cover report tampering and unsafe manifest paths.
