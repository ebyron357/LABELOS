...
- Immutable-style release directories containing copied artwork, validation report, manifest,
  and SHA-256 checksums. Package verification fails closed for malformed schema, unsafe paths,
  symlinks, mismatched byte counts/checksums, or a non-passing/inconsistent validation report.
...
Verified on 2026-08-10 from commit `3c4ce1d`:
...
generated in `dist/`; and an end-to-end package was created and checksum-verified under
`/tmp/labelos-e2e.*`. Release package verification now rejects malformed schema, unsafe paths,
symlinks, byte-count/checksum tampering, and validation reports that are not both passing and
