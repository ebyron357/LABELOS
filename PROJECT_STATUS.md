...
- Package verification rejects unknown manifest schemas, unsafe filenames, malformed checksums or
  byte counts, tampered files, non-passing reports, and specifications that do not reference the
  packaged artwork.
...
Verified on 2026-08-10 from commit `da56ddabb75975db2dc51db9f397b8ea2fb51deb`:
...
Results: 11 tests passed; Ruff passed; the sdist and wheel were generated in `dist/`; and the
end-to-end package was created and integrity-verified in a temporary directory. The package
contains `passing-label.svg`, `validation-report.json`, `label-spec.json`, and `manifest.json`.
`doctor` confirmed PyMuPDF and ZXing-C++ are available; Callas pdfToolbox remains unavailable.
QR and Code 128 regression tests generate raster, SVG, and PDF fixtures and verify their
decoded expected values. Manifest regression tests cover safe filenames, valid entry byte counts,
schema versioning, and a passing validation report. GitHub Actions runs tests, lint, and builds
