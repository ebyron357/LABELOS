...
| Linked (non-embedded) SVG raster files | **AVAILABLE NOW** (local, non-symlink paths below the SVG directory are DPI-checked and copied into release packages) |
...
Verified on 2026-09-14 from branch `cursor/label-production-system-readiness-a2a2`:

```text
python -m pytest -q                      # 61 passed; 2 upstream deprecation warnings
python -m ruff check .                   # passed
python -m compileall -q labelos illustrator_bridge tests  # passed
python -m pip check                      # passed
python -m build                          # passed; sdist and wheel in dist/
python -m labelos doctor --json          # passed; Callas SKIPPED_NOT_CONFIGURED
python -m labelos validate examples/label.json --json          # PASS
python -m labelos validate examples/failing-label.json --json  # REQUIRED_COPY_MISSING (expected)
python -m labelos package examples/label.json storage/demo-release  # PASS
python -m labelos verify-package storage/demo-release          # PASS
# after tampering artwork: checksum + byte-count mismatch, FAIL
```

The complete output is retained at
`/opt/cursor/artifacts/linked_svg_raster_final_verification.txt`. Callas pdfToolbox remains
unavailable and is never reported as PASS.
