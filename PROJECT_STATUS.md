...
| Linked (non-embedded) SVG raster files | **AVAILABLE NOW** (safe relative local assets are decoded, DPI-checked, checksummed, and included in release packages; remote, absolute, traversal, query/fragment, and symlink paths fail closed) |
...
Verified on 2026-08-19 at revision `9599840` on branch
`cursor/label-production-system-readiness-d29e`:

```text
python3 -m pytest -q                     # 63 passed (one upstream deprecation warning)
python3 -m ruff check .                  # passed
python3 -m compileall -q labelos illustrator_bridge tests
python3 -m pip check                     # passed
python3 -m build                         # sdist and wheel in dist/, no setuptools package warning
...
python3 -m labelos package examples/label.json /tmp/labelos-production-check --json
python3 -m labelos verify-package /tmp/labelos-production-check --json  # PASS
# linked-SVG fixture: local asset copied into package and checksum verified
```
