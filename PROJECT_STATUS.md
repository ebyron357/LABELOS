...
- Immutable-style release directories containing copied artwork, a canonical package-local
  label specification, validation report, manifest, and SHA-256 checksums.
- Fail-closed package verification: schema, regular package-local files, lowercase SHA-256
  checksums, byte counts, passing-report status, and report/spec/manifest agreement are
  all enforced.
...
Verified on 2026-08-10 from the worktree based on commit
`1e86abd006ee0fb53df88f1eecb61c73c05c342d`:
...
Results: 12 tests passed; Ruff and compilation passed; the sdist and wheel were generated in
`dist/`; and the end-to-end package was created and integrity-verified at
`/tmp/labelos-e2e`.
