...
python3 -m labelos validate examples/failing-label.json --json  # REQUIRED_COPY_MISSING, exit 1
...
The linked-SVG-raster production gap is complete: local non-symlink links below the SVG
directory are decoded and effective-DPI checked; their original relative paths and
checksums are included in schema-2 package manifests. Schema-1 package verification
remains supported. Next work should be limited to newly discovered defects or genuinely
