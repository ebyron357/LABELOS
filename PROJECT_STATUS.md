...
- Immutable-style release directories containing copied artwork, validation report, manifest,
  canonical package-local label specification, byte counts, and SHA-256 checksums.
- Package verification fails closed for an invalid manifest schema, non-local or symlinked
  package entries, malformed digests, byte/checksum mismatches, or a report that does not
  record a passing validation for the packaged specification.
- Malformed PDF artwork produces a `PDF_INVALID` validation report rather than a CLI traceback.
...
Verified on 2026-08-10 from code commit `56de6a3`:
...
`/tmp/labelos-e2e`. The release package contains `passing-label.svg`, `label-spec.json`,
`validation-report.json`, and `manifest.json`.
