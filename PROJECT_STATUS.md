...
| Linked (non-embedded) SVG raster files | **AVAILABLE NOW** (safe relative regular files are DPI-checked, release-packaged, and checksum-verified; remote, absolute, traversal, missing, and symlink references fail closed) |
...
Verified on 2026-08-19 from branch `cursor/label-production-system-readiness-e18d`:
...
Linked SVG assets are recorded in release manifest schema 2 and stored under
`assets/`. Package verification remains compatible with schema 1 release packages.
