...
python3 -m build --outdir /tmp/labelos-production-build-20260816
python3 -m labelos.cli validate examples/label.json --json
python3 -m labelos.cli package examples/label.json /tmp/labelos-production-e2e-20260816 --json
python3 -m labelos.cli verify-package /tmp/labelos-production-e2e-20260816 --json
python3 -m labelos.cli doctor --json
```

Results: 16 tests passed; Ruff, compilation, and dependency checks passed. The sdist and wheel
were generated in `/tmp/labelos-production-build-20260816`; both source-tree and built-wheel
package/verify workflows passed at `/tmp/labelos-production-e2e-20260816` and
`/tmp/labelos-wheel-e2e-20260816`. `doctor` confirmed PyMuPDF and ZXing-C++ are available;
Callas pdfToolbox remains unavailable. QR and Code 128 regression tests generate raster, SVG,
and PDF fixtures and verify their decoded expected values. Safe-area tests cover passing SVG
and failing PNG/PDF artwork; package tests cover checksum, byte-count, path-traversal, and
unexpected-file rejection. GitHub Actions runs tests, lint, and builds on Python 3.10 and 3.12.
