...
| Linked (non-embedded) SVG raster files | **AVAILABLE NOW** (safe local relative paths are decoded and effective-DPI validated; remote, absolute, traversal, query/fragment, missing, and symlink paths fail closed) |
...
Verified on 2026-08-19 from branch `cursor/label-production-system-readiness-0761`:

```text
python3 -m pytest -q                     # 61 passed (one upstream FastAPI/Starlette deprecation warning)
...
Release packages use manifest schema 2 when the artwork contains linked SVG rasters:
the referenced relative paths and their checksums are included in the package and verified.
Schema-1 packages remain verifiable for backward compatibility.
