...
| Linked (non-embedded) SVG raster files | **AVAILABLE NOW** (local relative regular rasters are DPI-checked, packaged, and checksummed; unsafe links fail closed) |
...
Verified on 2026-08-19 from branch `cursor/label-production-system-readiness-432d`:

```text
python3 -m pytest -q                     # 65 passed (one third-party deprecation warning)
...
python3 -m labelos.cli package examples/label.json <temporary-release> --json
python3 -m labelos.cli verify-package <temporary-release> --json    # PASS
# linked SVG-raster and package-tamper regression cases are covered in pytest
```
