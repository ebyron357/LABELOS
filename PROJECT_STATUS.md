...
- Immutable-style release directories containing copied artwork, validation report, manifest,
  a canonical package-local label specification, and SHA-256 checksums.
- Package verification rejects malformed manifests and unsafe filenames, and verifies schema
  version, byte counts, and checksums for every packaged file.
...
Verified on 2026-08-10 from commit `963344f`:
...
python3 -m labelos.cli package examples/label.json /tmp/labelos-release-check --json
python3 -m labelos.cli verify-package /tmp/labelos-release-check --json
python3 -m labelos.cli doctor --json
```

Results: 10 tests passed; Ruff passed; the sdist and wheel were generated in `dist/`; and the
end-to-end package was created and checksum-verified at `/tmp/labelos-release-check`. The
release contains `passing-label.svg`, `validation-report.json`, `label-spec.json`, and
`manifest.json`; manifest verification also covers byte counts and safe package-local paths.
