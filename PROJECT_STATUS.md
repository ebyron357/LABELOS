...
- Invalid PDF artwork is reported as a `PDF_INVALID` validation error rather than crashing the
  operator CLI.
...
- Immutable-style release directories containing copied artwork, normalized label specification,
  ...
  unexpected files, symlinks/non-regular artifacts, unsafe paths, duplicate manifest entries,
  broken artwork bindings, and reports that do not pass.
...
Verified on 2026-08-15 from the release-package schema v2 working tree:
...
Results: 11 tests passed; Ruff, compileall, and pip check passed. The sdist and wheel are
created under `/tmp/labelos-production-build`; the end-to-end package is at
`/tmp/labelos-final-e2e` and its schema v2 manifest is checksum- and byte-count-verified.
