...
- Safe-area artwork enforcement for SVG, PNG, and PDF. It permits uniform full-bleed
  ...
  when artwork cannot be rendered or its edge background is ambiguous.
- Malformed PDFs return a structured `PDF_INVALID` validation error instead of terminating the
  operator workflow.
...
Verified on 2026-08-15 during the safe-area enforcement implementation:
...
Results: 14 tests passed, including SVG, PNG, and PDF safe-area failures plus malformed-PDF
handling; Ruff passed; the sdist and wheel were generated in `/tmp/labelos-production-build`;
and the end-to-end package was created and checksum-verified at `/tmp/labelos-production-e2e`.
