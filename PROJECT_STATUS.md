...
| Linked (non-embedded) SVG raster files | **AVAILABLE NOW** (plain, local, non-symlink relative links beneath the SVG directory are decoded, DPI-checked, packaged, and checksummed) |
...
Verified on 2026-09-06 from branch `cursor/label-production-system-readiness-dbca`:

```text
python3 -m pytest -q                     # 61 passed (2 upstream deprecation warnings)
python3 -m ruff check .                  # passed
python3 -m compileall -q labelos illustrator_bridge tests
python3 -m pip check                     # passed
python3 -m build                          # sdist + wheel passed
python3 -m labelos --help                # passed
...
python3 -m labelos package examples/label.json /tmp/labelos-e2e-release --json
