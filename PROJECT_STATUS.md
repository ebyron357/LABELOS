...
- Immutable-style release directories containing copied artwork, a canonical package-local
  label spec, validation report, manifest, and SHA-256 checksums.
- Package verification rejects unsupported manifest schemas and unsafe filenames, then checks
  every packaged file's SHA-256 checksum and byte count.
...
Verified on 2026-08-10 before the next delivery commit:
...
decoded expected values. Package tests confirm the canonical `label-spec.json` is included and
that a manifest cannot use `../` to escape its release directory. GitHub Actions runs tests,
