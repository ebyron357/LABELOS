...
- Safe-area validation that fails closed: PNGs are inspected directly, SVG/PDF artwork is
  rendered at 300 DPI, transparent pixels are ignored, and visible output outside the
  bleed-plus-safe-area inset is rejected.
- Operator CLI: validate, package, verify-package, and doctor.
- Immutable-style release directories containing copied artwork, validation report, manifest,
  canonical label specification, SHA-256 checksums, and byte counts. Verification rejects
  traversal paths, symlinks, malformed JSON, failed reports, and inconsistent metadata.
- Passing and failing fixture coverage plus safe-area, CLI, package-integrity, QR, and barcode
  regression tests.
...
Verified on 2026-08-11 from commit `19d81e42a922f4bb436c552e649a5d8894b4c750`:
...
Results: 22 tests passed; Ruff passed; the sdist and wheel were generated in
`/tmp/labelos-production-build`; and the end-to-end package was created and
checksum-verified at `/tmp/labelos-production-e2e`.
`doctor` confirmed PyMuPDF and ZXing-C++ are available; Callas pdfToolbox remains unavailable.
QR and Code 128 regression tests generate raster, SVG, and PDF fixtures and verify decoded
expected values. Safe-area fixtures cover boundaries, transparent pixels, SVG transforms, and
PDF output. GitHub Actions runs tests, lint, and builds on Python 3.10 and 3.12.
