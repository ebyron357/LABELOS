...
| Linked (non-embedded) SVG raster files | **AVAILABLE NOW** (safe local `href` files are decoded, DPI-checked, included in release packages, and checksum-verified; remote, absolute, traversal, missing, and symlink references fail closed) |
...
Verified on 2026-08-19 from branch `cursor/label-production-system-readiness-a082`:

```text
python -m pytest -q                      # 60 passed (one FastAPI/httpx deprecation warning)
...
The wheel includes the `python -m labelos` entry point. Linked SVG rasters are
DPI-checked and packaged with a schema-2 manifest; their checksums and byte counts
are verified. Schema-1 packages remain verifiable.
