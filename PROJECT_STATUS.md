...
- Manifest schema v2 verification rejects unexpected files, symlinks and non-regular artifacts,
  unsafe paths, invalid metadata names, tampered byte counts/checksums, unbound specifications,
  and reports that do not record a passing result.
- Invalid PDF input returns a `PDF_INVALID` validation error instead of crashing the CLI.
...
Verified on 2026-08-15 from code commit `be1e718`:
...
python3 -m build --outdir /tmp/labelos-production-build-GLgjp6
python3 -m labelos.cli validate examples/label.json --json
python3 -m labelos.cli package examples/label.json /tmp/labelos-final-e2e-IGU0aP/release --json
python3 -m labelos.cli verify-package /tmp/labelos-final-e2e-IGU0aP/release --json
python3 -m labelos.cli doctor --json
```

Results: 13 tests passed; Ruff, bytecode compilation, and dependency consistency checks passed;
the sdist and wheel were generated in `/tmp/labelos-production-build-GLgjp6`; and a schema-v2
end-to-end package was created and checksum-verified at `/tmp/labelos-final-e2e-IGU0aP/release`.
