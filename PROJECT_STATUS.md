...
- JSON label specification validation with physical dimensions, bleed, content safe-area
  checks for SVG/PDF, minimum DPI, and required-copy fields. Raster safe areas fail closed
  when the critical content cannot be separated from the flattened background.
...
- Immutable-style release directories containing copied artwork, validation report, manifest,
  SHA-256 checksums, and byte counts. Verification rejects malformed manifests, filename
  traversal, unexpected files, subdirectories, and symlinks.
