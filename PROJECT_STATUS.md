...
| Linked (non-embedded) SVG raster files | **AVAILABLE NOW** (safe local files are DPI-checked and bundled with their release package) |
...
Verified on 2026-08-19 from branch `cursor/label-production-system-readiness-5af7`:

```text
python -m pytest -q                      # 63 passed; one FastAPI/httpx deprecation warning
