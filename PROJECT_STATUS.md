...
- Malformed PNG, SVG, and PDF inputs fail closed with a structured format error; dependent
  copy, code, and safe-area checks do not run against unreadable artwork.
...
Verified on 2026-08-18 from code revision `4d42e83`:
...
python3 -m build --outdir /tmp/labelos-final-build-pDSvqX
                                         # sdist and wheel created
python3 -m labelos.cli validate examples/label.json --json
python3 -m labelos.cli package examples/label.json /tmp/labelos-final-package-WeeED0 --json
python3 -m labelos.cli verify-package /tmp/labelos-final-package-WeeED0 --json
python3 -m labelos.cli doctor --json
python3 -m pip install --no-deps --target /tmp/labelos-final-wheel-8jgTZu \
  /tmp/labelos-final-build-pDSvqX/labelos-0.1.0-py3-none-any.whl
PYTHONPATH=/tmp/labelos-final-wheel-8jgTZu python3 -m labelos.cli validate \
  examples/label.json --json
PYTHONPATH=/tmp/labelos-final-wheel-8jgTZu python3 -m labelos.cli package \
  examples/label.json /tmp/labelos-final-package-WeeED0 --json
PYTHONPATH=/tmp/labelos-final-wheel-8jgTZu python3 -m labelos.cli verify-package \
  /tmp/labelos-final-package-WeeED0 --json
```

Results: 15 tests passed; Ruff, bytecode compilation, and dependency consistency checks
passed; the sdist and wheel were generated in `/tmp/labelos-final-build-pDSvqX`; and the
end-to-end package was created and integrity-verified at `/tmp/labelos-final-package-WeeED0`.
