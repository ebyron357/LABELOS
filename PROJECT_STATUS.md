...
| Linked (non-embedded) SVG raster files | **AVAILABLE NOW** (relative regular files beneath the SVG directory; checked for effective DPI and checksummed in release packages) |
...
Current implementation verification (2026-09-14, Python 3.12):
...
python3 -m labelos validate examples/failing-label.json --json  # expected exit 1
...
# tampered packaged artwork: verify-package fails checksum validation
```

Previously verified on 2026-08-18 from branch `stabilize/canonical-validator`:
