...
- Safe-area enforcement across SVG, PNG, and PDF: visible artwork must remain inside the
  configured post-trim inset. Uniform full-bleed backgrounds are excluded; ambiguous edges
  fail closed.
...
- Immutable-style release directories containing copied artwork, validation report, manifest,
  SHA-256 checksums, byte counts, and a captured validation specification.
- Package verification rejects unsafe manifest paths, symlinked/non-regular payloads,
  inconsistent reports/specs, and unexpected files.
...
SVG, and PDF artwork. Package regression tests cover checksum and byte-count mismatch, POSIX
and Windows-style path traversal, and unexpected files. GitHub Actions runs tests, lint, and
